...
FreqNgr <- ngrams(data.dir, sampSize = 1e5, ngrO = 1, mode = "run")
...
sqldf("CREATE UNIQUE INDEX `gram1_w1_index` ON `gram1` (`word1` ASC)", dbname = "ngrams")
...
result1 <- ngrams(data.dir, sampSize = 1e4, ngrO = 2, mode = "test")

# "select count(word1) from gram2"

result2 <- ngrams(data.dir, sampSize = 1e4, ngrO = 2, mode = "test")
...
results <- ngrams(data.dir, sampSize = 1e5, ngrO = 2, mode = "run")
save(results, file = "3grams.perf.graph.rdata")

results <- ngrams(data.dir, sampSize = 1e5, ngrO = 3, mode = "run")
save(results, file = "3grams.perf.graph.rdata")

results <- ngrams(data.dir, sampSize = 1e5, ngrO = 4, mode = "run")
save(results, file = "4grams.perf.graph.rdata")

results <- ngrams(data.dir, sampSize = 1e5, ngrO = 5, mode = "run")
