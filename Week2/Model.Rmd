...
FreqNgr <- ngrams(path, sampSize = 1e5, ngrO = 1, env = "run")
...
sqldf("CREATE INDEX `Words_index` ON `words` (`word` ASC)", dbname = "ngrams")
...
4) store only unique words (1-grams) and for all other grams - tables with indices of words. maybe scan all the files for unique words first so I will have indices for all the words in data and then construct all n>1grams based on it. 
...
result <- ngrams(path, sampSize = 1e4, ngrO = 2, env = "test")

# "select count(word1) from gram2"

ngrams(path, sampSize = 1e4, ngrO = 2, env = "test")
...
```{r}
results <- ngrams(path, sampSize = 5e4, ngrO = 2, env = "run")
# tmp <- 
# FreqNgr5 <- tmp[[1]]
